...
int main(int argc, char* argv[]) {
  ...
  //construct area
  area = new Area(GRID, GRID);
  
  fillTiles();
  glutMainLoop();
  
  delete area;
  for(i=0;i<nTileObjs;i++){
    delete tileObjs[i];
  }
  return 0;
}
...
//PUBLIC FUNCTIONS
extern tile_t* setTile(int i, int j, int k, tile_t t){
  tiles[i][j][k]=t;
  return &tiles[i][j][k];
}
...
void fillTiles(){
  ...
  for(i=0;i<GRID;i++)
    for(j=0;j<GRID;j++){
      tileObjs[nTileObjs++] = new Tile(i,j,1,1,area);
    }
    
  //happy man on layer 0!
  moveyDude = new Mover(6,6,0,2,area);
}
...
void specialkey(int key, int mouseX, int mouseY){
  switch(key){
    case GLUT_KEY_LEFT : moveyDude.move(-1,0,0); break;
    case GLUT_KEY_RIGHT : moveyDude.move(1,0,0); break;
    case GLUT_KEY_UP : moveyDude.move(0,1,0); break;
    case GLUT_KEY_DOWN : moveyDude.move(0,-1,0); break;
  }
  glutPostRedisplay();
}
